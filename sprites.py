...
class Player(Sprite):
    ...
    def load_images(self):
        self.standing_frames = [self.game.spritesheet.get_image(382, 635, 174, 126),
                                self.game.spritesheet.get_image(0, 1456, 216, 101)
                                ]
        for frame in self.standing_frames:
            frame.set_colorkey(BLACK)
        self.walk_frames_r = [self.game.spritesheet.get_image(0, 1879, 206, 107),
...
class Platform(Sprite):
    def __init__(self, game, x, y):
        ...
        if random.randrange(100) < POW_SPAWN_PCT:
            Pow(self.game, self)
        if random.randrange(100) < BOOM_SPAWN_PCT:
            Boom(self.game, self)
        # if random.randrange(100) < POW_SPAWN_PCT:
...
class Boom(Sprite):
    def __init__(self, game, plat):
        # allows layering in LayeredUpdates sprite group
        self._layer = BOOM_LAYER
        # add a groups property where we can pass all instances of this object into game groups
        self.groups = game.all_sprites, game.powerups
        Sprite.__init__(self, self.groups)
        self.game = game
        self.plat = plat
        self.type = random.choice(['boom'])
...
#         Sprite.__init__(self, self.groups)
#         self.game = game
#         self.image_up = self.game.spritesheet.get_image(232, 1390, 95, 53)
#         self.image_up.set_colorkey(BLACK)
#         self.image_down = self.game.spritesheet.get_image(885, 752, 51, 87)
#         self.image_down.set_colorkey(BLACK)
#         self.image = self.image_up
#         self.image.set_colorkey(BLACK)
#         self.rect = self.image.get_rect()
